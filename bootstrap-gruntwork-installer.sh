#!/bin/bash
#
# A bootstrap script to install the nholuongut Installer.
#
# Usage:
#
...
# Rationale:
#
# The nholuongut Installer makes installing nholuongut Script Modules as easy as installing a package using apt-get,
# brew, or yum. However, something has to install the nholuongut Installer first. One option is for each nholuongut
...
#
# In your Packer and Docker templates, you can use the above one-liner to install the nholuongut Installer, and then
# start using the `nholuongut-install` command immediately.
...
readonly FETCH_DOWNLOAD_URL_BASE="https://github.com/nholuongut-io/fetch/releases/download"
readonly FETCH_INSTALL_PATH="$BIN_DIR/fetch"

readonly nholuongut_INSTALLER_DOWNLOAD_URL_BASE="https://raw.githubusercontent.com/nholuongut/nholuongut-installer"
readonly nholuongut_INSTALLER_INSTALL_PATH="$BIN_DIR/nholuongut-install"
readonly nholuongut_INSTALLER_SCRIPT_NAME="nholuongut-install"

function print_usage {
  echo
  echo "Usage: bootstrap-nholuongut-installer.sh [OPTIONS]"
  echo
  echo "A bootstrap script to install the nholuongut Installer ($nholuongut_INSTALLER_SCRIPT_NAME)."
  echo
  echo "Options:"
  echo
  echo -e "  --version\t\tRequired. The version of $nholuongut_INSTALLER_SCRIPT_NAME to install (e.g. v0.0.38)."
  ...
  echo -e "  --download-url\tOptional. The URL from where to download $nholuongut_INSTALLER_SCRIPT_NAME. Mostly used for automated tests. Default: $nholuongut_INSTALLER_DOWNLOAD_URL_BASE/(version)/$nholuongut_INSTALLER_SCRIPT_NAME."
  echo -e "  --no-sudo\tOptional. When true, don't use sudo to install binaries. Default: false."
  echo
  echo "Examples:"
  echo
  echo "  Install version v0.0.38:"
  echo "    bootstrap-nholuongut-installer.sh --version v0.0.38"
  echo
  echo "  One-liner to download this bootstrap script from GitHub and run it to install version v0.0.38:"
  echo "    curl -Ls https://raw.githubusercontent.com/nholuongut/nholuongut-installer/v0.0.38/bootstrap-nholuongut-installer.sh | bash /dev/stdin --version v0.0.38"
}
...
function download_url_to_file {
  local -r url="$1"
  local -r file="$2"
  local -r tmp_path=$(mktemp "/tmp/nholuongut-bootstrap-download-XXXXXX")
  local -r no_sudo="$3"
...
function install_nholuongut_installer {
  local -r install_path="$1"
  local -r version="$2"
  local -r download_url="$3"
  local -r no_sudo="$4"

  echo "Installing $nholuongut_INSTALLER_SCRIPT_NAME version $version to $install_path"
...
This folder is not an industry standard, but a convention we use at  nholuongut (http://www.nholuongut) make User Data
scripts manageable. Many of the modules in Script Modules (https://github.com/nholuongut-io/script-modules) need not
...
function bootstrap {
  ...
  if [[ -z "$download_url" ]]; then
    download_url="${nholuongut_INSTALLER_DOWNLOAD_URL_BASE}/${installer_version}/${nholuongut_INSTALLER_SCRIPT_NAME}"
  fi

  echo "Installing $nholuongut_INSTALLER_SCRIPT_NAME..."
  install_fetch "$FETCH_INSTALL_PATH" "$fetch_version" "$no_sudo"
  install_nholuongut_installer "$nholuongut_INSTALLER_INSTALL_PATH" "$installer_version" "$download_url" "$no_sudo"
